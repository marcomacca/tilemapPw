...
def trafficLightPolicy(TrafficLightAxis_waiting, TrafficLightAxis_active, green_remaining_time, axis_pop_diff_check, minimum_pop_length):
    ...
    while green_remaining_time > MINIMUM_GREEN_LIGHT_DURATION:
        if axis_population_waiting >= minimum_pop_length \
        & axis_population_waiting >= axis_population_active * axis_pop_diff_check:
            ...
            if green_remaining_time < MINIMUM_GREEN_LIGHT_DURATION:
                green_remaining_time = MINIMUM_GREEN_LIGHT_DURATION
...
def trafficLightGreenLifeCycle(TrafficLightAxis1, TrafficLightAxis2, green_remaining_time, green_elapsed_time):
    if green_remaining_time < MIN_UNIT_OF_TIME:
        ...
        if green_remaining_time == 0:
            #end of green life cycle
            TrafficLightAxis1.sum_of_green_duration += green_elapsed_time
...
def main():
    ...
    for i in range(FIRST_TIMESLOT_ID, LAST_TIMESLOT_ID + 1):
        ...
        & tl_y.car_input2 / MIN_UNIT_OF_TIME * TIME_THRESHOLD_FOR_FLASHING_LIGHTS < MIN_VEHICLE_INPUT_PER_FLASHING_LIGHTS_THRESHOLD \
        & tl_x.car_input1 / MIN_UNIT_OF_TIME * TIME_THRESHOLD_FOR_FLASHING_LIGHTS < MIN_VEHICLE_INPUT_PER_FLASHING_LIGHTS_THRESHOLD \
        & tl_x.car_input2 / MIN_UNIT_OF_TIME * TIME_THRESHOLD_FOR_FLASHING_LIGHTS < MIN_VEHICLE_INPUT_PER_FLASHING_LIGHTS_THRESHOLD:
        ...
        while timeslot_remaining_time > 0:
            ...
            while green_remaining_time > 0:
                if tl_x.light_status == 'RED':
                    trafficLightPolicy(tl_x, tl_y, green_remaining_time,min_pop_for_policy_check, diff_thres_for_policy_check)
                    trafficLightGreenLifeCycle(tl_y, tl_x, green_remaining_time, green_elapsed_time)
                elif tl_y.light_status == 'RED':
                    trafficLightPolicy(tl_y, tl_x, green_remaining_time,min_pop_for_policy_check, diff_thres_for_policy_check)
